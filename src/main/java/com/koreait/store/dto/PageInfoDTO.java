...
public class PageInfoDTO<T> {
    @Setter private Integer categoryNo = 1; // 조회되는 카테고리 번호
    ...
    @Setter private Integer totalPageCount; // 총 페이지 개수(조회된 elem과 상관 없음)
    ...
    @Setter private List<T> elements; // Pagenation 대상 요소들

    // database에 조회되는 elements 의 offset (이 번호 이후의 element조회)
    public Integer getOffset() {
        return (page - 1 ) * size; // 몇번 이후의 데이터를 가져올지 계산
    }
