...
public class SecurityOAuth2UserService extends DefaultOAuth2UserService {
    ...
    // 원래는 소셜 로그인 시 ci를 가져와야 하지만, 테스트용에서는 ci를 가져올 수 없으므로, 임의로 만들어 줌
    // 운영 단계에서는 사용하지 않아야 함

//    private final String CI = "TEST_CI"; // 빠른 TEST용
    private final String CI = "TEST_CI2"; // 빠른 TEST용
//    private final String CI ="1ZPf5Q/9zIIDyoRuH7O6vCZymwiYA0kz0Dao/W7YVWCuPFEXI8WIWPslm/GwPUH6VOpd2vqktH1hLB3CxveR3w==";
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if(Objects.isNull(snsInfoDTO)){
            throw new OAuth2AuthenticationException("INVALIDATED SNS");
        }

        // 같은 ci를 가지는 유저를 DB에서 찾는다
        UserDTO existsUser = userMapper.selectUserByCi(snsInfoDTO.getCi());
        log.info(existsUser);
        // 기존에 해당 ci를 가지는 유저가 없으면 null반환이 아니라
        ...
        // 기존에 이 SNS가 변동된 적이 없다면
        if (!isSnsExists) {
            // 기존에 유저가 있다면 SNS 정보를 유저에게 넣어준다
